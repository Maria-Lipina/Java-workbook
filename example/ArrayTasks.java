...
public class ArrayTasks {
     ...
     * n ≤ 110000, m ≤ 1000.
     * @param a массив, в котором идет поиск элементов, ближайших по значению
     * @param b массив, относительно которого проводится поиск "близнецов".
     * @return массив длины, равной массиву B, в котором хранятся индексы элементов А, ближайших по значению к B
     */

    public static int[] findSimilar(int[] a, int [] b) {
        int [] result = new int[b.length];
        int diff = 0;
        for (int i = 0; i < b.length; i++) {
            diff = Math.abs(compare(diff, b[i]));
            result[i] = diff;
        }
    ...
    private static int compare(int prevDiff, int bElement) { //индекс предыдущей разницы (типа ближайший) и значение элемента, с которым нужно сравнить сейчас, двигаясь вперед по массиву A
        return 0; //тут должен быть индекс элемента в а, ближайшего по значению
    }
}
